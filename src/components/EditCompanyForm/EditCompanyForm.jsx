...
const EditCompanyForm = ({ companyId, onClose }) => {
  ...
  useEffect(() => {
    const fetchCompanyDetails = async () => {
      try {
        const response = await axios.get(`http://localhost:4804/api/Companies/${companyId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:4804/api/Companies/${companyId}`, formData);
